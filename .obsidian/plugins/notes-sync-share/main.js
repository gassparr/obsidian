...
var SettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("User").setDesc("Please enter your username and password, if it does not exist, it will be created automatically.").addText((text) => text.setPlaceholder("username").setValue(this.plugin.settings.username).onChange(async (value) => {
...
var NoteSyncSharePlugin = class extends import_obsidian5.Plugin {
  ...
  async syncNotesByDiff(diff) {
    ...
    const uploadFetchPromises = diff.clientDiffActionInfo.uploadFiles.map(async (f) => {
      ...
      if (file) {
        const isDir = !(file instanceof import_obsidian5.TFile);
        const formData = new FormData();
    ...
    await request(`${this.getServerUrl()}/sync/syncCompleted`, {
      ...
      new import_obsidian5.Notice("Synchronized notes completed");
    });
  }
  async syncNotes() {
    new import_obsidian5.Notice("Start syncing notes");
    ...
    await request(`${this.getServerUrl()}/sync/diff`, {
      ...
      (data) => this.syncNotesByDiff(data)
    );
  }
  async onload() {
